                ...
                {report.replies && report.replies.length > 0 ? (
                    <div className="space-y-4 mb-6">
                        {report.replies.map((reply, index) => (
                            <div
                                key={index}
                                className={`flex gap-3 ${reply.authorType === 'lawyer' ? 'flex-row-reverse' : 'flex-row'}`}
                            >
                                <div className={`p-2 rounded-full ${reply.authorType === 'lawyer' ? 'bg-green-100' : 'bg-blue-100'}`}>
                                    <UserIcon className={`w-4 h-4 ${reply.authorType === 'lawyer' ? 'text-green-600' : 'text-blue-600'}`} />
                                </div>
                                <div className={`flex-1 ${reply.authorType === 'lawyer' ? 'text-right' : 'text-left'}`}>
                                    <div className={`inline-block max-w-[80%] p-3 rounded-lg ${reply.authorType === 'lawyer'
...
export default LawyerReportThread;
