...
const ClientDetails = () => {
    ...
    const handleDeleteClient = async () => {
        ...
        try {
            ...
            if (response.ok) {
                navigate(`/${userRole.toLowerCase()}/clients`);
            }
        } catch (err) {
    ...
    if (!client) {
        return (
            <div className="p-8 text-center">
                ...
                <button
                    onClick={() => navigate(`${getBasePath()}/clients`)}
                    className={`mt-4 px-4 py-2 bg-${getPrimaryColor()}-600 text-white rounded-lg hover:bg-${getPrimaryColor()}-700`}
                >
                    Back to Clients
                </button>
            </div>
    ...
    return (
        <div className={userRole === 'HOC' ? 'p-6' : ''}>
            ...
            {/* Admin-only: Client Portal Link Section */}
            {userRole === 'Admin' && (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className={`flex ${userRole === 'HOC' ? 'justify-between' : ''} items-center mb-4`}>
                    <h3 className="text-lg font-semibold text-gray-900">Cases</h3>
                    {/* HOC-only: Add Case Button */}
                    {userRole === 'HOC' && (
                        <button
                ...
                {cases.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {cases.map((caseItem) => {
                            const isAssignedToMe = userRole === 'HOC' && caseItem.assignedTo && (
                ...
                {complaints.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {complaints.map((complaint) => (
                            <div
                                key={complaint._id}
                                onClick={() => navigate(`${getBasePath()}/complaints/${complaint._id}`)}
